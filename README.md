...
- **웹 검색**: 입력창 위의 토글을 켜면 `plugins: [{"id": "web", "max_results": 5}]`가 요청에 추가됩니다. 검색 결과 출처가 답변 아래에 표시됩니다. 강화 검색과 무관하게 항상 켜고 끌 수 있습니다.
- **강화 검색**: 사이드바 "새 대화" 아래 녹색 토글(기본 ON, 설정은 브라우저에 기억됨). 켜면 입력창 위에 "🔍 강화 검색" 배지가 표시됩니다. 각 질문을 ① 영어로 번역(원문이 영어면 생략) → ② 3가지 규칙(출처 필수·웹 검색 검증·국가별 다국어 병행 검색)을 적용해 답변 생성(웹 검색 토글이 켜져 있을 때만 실제 웹 검색 수행) → ③ 결과를 원문 언어로 역번역해 표시합니다. 번역엔 `.env`의 `TRANSLATE_MODEL`, 답변 생성엔 선택한 대화 모델을 사용합니다. 이미지 첨부 시엔 자동으로 일반(비강화) 방식으로 동작합니다.
